...
import { DollarSign, PiggyBank, Brain, ArrowRight } from 'lucide-react';
import { GeminiCoach } from './GeminiCoach';

function getSymbolMultiplier(value: string | number): number {
  const text = String(value ?? '').trim();
  switch (text) {
    case '+++': return 3; case '++': return 2; case '+': return 1;
...
export const GameBoard = ({ playerCharacter, cards, onGameEnd }: GameBoardProps) => {
  ...
  const advanceGame = () => {
    ...
    setIsProcessing(false); setLastChoiceContext(null); setHasAskedCoach(false);
    setAutoExplainCoach(false); setPlayerReaction('neutral');
    if (currentCardIndex + 1 < cards.length) setCurrentCardIndex(prev => prev + 1);
    else finishGame(currentScore);
  };
  ...
  const handleChoice = async (choice: Choice, choiceIndex: number) => {
    ...
    setIsProcessing(true); setPlayerReaction('thinking');
    ...
    setLastChoiceContext({
      situationTitle: activeCard.scenario, choiceText: choice.text,
      impact: choice.impactDescription || "Processed turn", quality,
    });
    ...
    try {
      ...
      setStats(updatedUser.stats); setCurrentScore(updatedUser.overallScore);
      if (isBestChoice) {
        setPlayerReaction('happy'); setDamageDealt(Math.min((updatedUser.overallScore - currentScore) * 2, 30));
        setEvilHealth(prev => Math.max(0, prev - (damageDealt || 10))); setBattleMessage('🏆 Perfect Choice!');
      } else if (isWorstChoice) {
        setIsEvilAttacking(true); setPlayerReaction('sad'); setBattleMessage(`😈 Oh no! ${evilCharacter?.name} is gaining power!`);
      } else { setBattleMessage('👍 Decent choice.'); }
      setTimeout(() => { setIsEvilAttacking(false); setDamageDealt(undefined); setPlayerReaction('neutral'); }, 2000);
      if (!isWorstChoice) {
        timerRef.current = setTimeout(() => { setIsProcessing(false); setBattleMessage(''); advanceGame(); }, 3000);
      } else { setIsProcessing(false); }
    } catch (error) { console.error("Turn failed", error); setIsProcessing(false); }
  };

  const finishGame = (finalScore: number) => { setGameOver(true); setTimeout(() => onGameEnd(finalScore), 2500); };
  ...
  return (
    <div className="min-h-screen w-full bg-background relative overflow-hidden font-sans">
      <div className="relative z-10 flex flex-col h-full max-w-7xl mx-auto p-3 sm:p-4 pb-24">
        <JourneyTrack currentRound={currentCardIndex} totalRounds={cards.length} character={playerCharacter} />

        {/* Scaled Header for Mobile */}
        <div className="flex justify-between items-center mb-4 sm:mb-6 px-1 sm:px-4 pt-2 gap-2">
          <div className="scale-75 sm:scale-100 origin-left">
            <PlayerCharacterComponent character={playerCharacter} reaction={playerReaction} />
          </div>
          <div className="flex flex-col items-center flex-1">
            <div className="bg-primary text-emerald-950 px-3 py-1 sm:px-5 sm:py-2 rounded-full font-bold text-[10px] sm:text-xs border border-primary/50 shadow-lg whitespace-nowrap">
              ROUND {currentCardIndex + 1}/{cards.length}
            </div>
          </div>
          <div className="scale-75 sm:scale-100 origin-right">
            {evilCharacter && <VillainCharacter character={evilCharacter} state={getVillainState()} health={evilHealth} maxHealth={evilMaxHealth} damage={damageDealt} />}
          </div>
        </div>

        {/* Stats Grid for Mobile */}
        <div className="grid grid-cols-3 gap-2 sm:flex sm:justify-center sm:gap-4 mb-6 sm:mb-8">
          <StatCard icon={<DollarSign className="w-3 h-3 sm:w-4 sm:h-4" />} label="Money" value={`$${stats.money}`} color={stats.money > 0 ? 'text-emerald-400' : 'text-red-400'} />
          <StatCard icon={<Brain className="w-3 h-3 sm:w-4 sm:h-4" />} label="Knowledge" value={`${stats.financeKnowledge}`} color="text-blue-400" />
          <StatCard icon={<PiggyBank className="w-3 h-3 sm:w-4 sm:h-4" />} label="Happy" value={`${stats.happiness}`} color="text-yellow-400" />
        </div>

        {!gameOver && activeCard && (
          <div className="flex-1 flex flex-col items-center justify-start w-full">
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} key={currentCardIndex} className="mb-4 sm:mb-6 text-center max-w-3xl w-full bg-emerald-950/60 p-4 sm:p-6 rounded-2xl border border-white/10 backdrop-blur-md shadow-2xl">
              <h2 className="font-display text-lg md:text-2xl leading-relaxed text-white drop-shadow-md">
                &quot;{activeCard.scenario}&quot;
              </h2>
            </motion.div>
            ...
            {(lastChoiceContext || hasAskedCoach) && (
              <div className="flex flex-col items-center gap-6 w-full mb-4">
                {lastChoiceContext && <GeminiCoach context={lastChoiceContext} onAsked={onAskCoach} autoExplain={autoExplainCoach} />}
                {hasAskedCoach && (
...
const StatCard = ({ icon, label, value, color }: { icon: any, label: string, value: any, color: string }) => (
  <div className="flex flex-col sm:flex-row items-center gap-1 sm:gap-3 px-2 py-2 sm:px-4 sm:py-3 rounded-xl bg-emerald-900/80 border border-white/5 min-w-0 sm:min-w-[140px] justify-center shadow-lg transition-transform">
    ...
    <div className="flex flex-col text-center sm:text-left overflow-hidden w-full">
      <span className="text-[8px] sm:text-[10px] text-emerald-200/60 font-bold uppercase tracking-wider truncate">{label}</span>
      <span className={cn("font-bold text-sm sm:text-lg leading-tight truncate", color)}>{value}</span>
    </div>
  </div>
);